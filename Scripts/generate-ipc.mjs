...
const IPC_DIR = resolve(root, 'Services/Core/Ipc');
const IPC_HANDLERS_DIR = resolve(root, 'Services/Core/Ipc/Handlers');
const OUTPUT = resolve(root, 'Frontend/src/lib/ipc.ts');

// Read all .cs files from Ipc/ and Ipc/Handlers/
const collectCsFiles = (dir) => {
...
const ipcFiles = [
  ...collectCsFiles(IPC_DIR),
  ...collectCsFiles(IPC_HANDLERS_DIR),
].sort();

const source = ipcFiles
...
L.push(`SOURCE: Services/Core/Ipc/ (@type + @ipc annotations)`);
L.push(`*/`);
L.push(``);
