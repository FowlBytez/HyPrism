...
HyPrism использует **data-driven систему зеркал** для загрузки игровых файлов. Вместо жёстко прописанных URL лаунчер читает определения зеркал из файлов `.mirror.json` в директории `Mirrors/`. Это руководство объясняет, как работает система, как настраивать существующие зеркала и как добавлять свои.
...
- [Быстрый старт: добавление своего зеркала](#быстрый-старт-добавление-своего-зеркала)
...
- [Примеры встроенных зеркал (с пояснениями)](#примеры-встроенных-зеркал-с-пояснениями)
...
HyPrism всегда сначала пытается скачать файлы с **официальных серверов Hytale**. Если официальная загрузка недоступна или медленная, лаунчер автоматически тестирует доступные зеркала и использует лучшее на основе приоритета и скорости.
...
1. При **первом запуске**, если директория `Mirrors/` не существует или пуста, лаунчер генерирует дефолтные файлы `.mirror.json` для встроенных зеркал сообщества (EstroGen, CobyLobby, ShipOfYarn).
2. При **каждом запуске** лаунчер читает все файлы `*.mirror.json` из директории `Mirrors/` и создаёт из них источники загрузки.
3. Зеркала с `"enabled": false` пропускаются.
4. Источники **сортируются по приоритету** (меньшее число = выше приоритет). Официальный источник Hytale имеет приоритет `0`, зеркала обычно начинаются с `100`.
5. Когда нужна загрузка, лаунчер автоматически выбирает лучший доступный источник.

> **Важно:** После первого запуска лаунчер никогда не перезаписывает и не перегенерирует существующие файлы зеркал. Все ваши изменения в `.mirror.json` файлах сохраняются.
...
<каталог-данных>/
└── Mirrors/
    ├── estrogen.mirror.json       ← встроенное, сгенерировано автоматически
    ├── cobylobby.mirror.json      ← встроенное, сгенерировано автоматически
    ├── shipofyarn.mirror.json     ← встроенное, сгенерировано автоматически
    └── my-custom.mirror.json      ← ваше пользовательское зеркало (опционально)
```
...
## Быстрый старт: добавление своего зеркала
...
**Реальный пример** — CobyLobby использует этот метод:
```json
{
  "method": "json-api",
...
API по адресу `https://cobylobbyht.store/launcher/patches/release/versions?os_name=linux&arch=x64` возвращает:
```json
{
  "items": [
...
**Реальный пример** — EstroGen использует nginx autoindex. HTML по адресу `https://licdn.estrogen.cat/hytale/patches/linux/x64/release/0/` выглядит так:
...
## Примеры встроенных зеркал (с пояснениями)

Лаунчер поставляется с тремя встроенными зеркалами. Вот их полные конфигурации с объяснениями.

### EstroGen (pattern + html-autoindex)

EstroGen хостит файлы на веб-сервере с включённым nginx autoindex. Лаунчер парсит HTML-листинг каталога для обнаружения версий.
...
| Тип | URL |
|-----|-----|
| Полная сборка | `https://licdn.estrogen.cat/hytale/patches/linux/x64/release/0/8.pwr` |
| Diff 7→8 | `https://licdn.estrogen.cat/hytale/patches/linux/x64/release/7/8.pwr` |
| Подпись | `https://licdn.estrogen.cat/hytale/patches/linux/x64/release/0/8.pwr.sig` |
| Список версий | `https://licdn.estrogen.cat/hytale/patches/linux/x64/release/0/` |

### CobyLobby (pattern + json-api)

CobyLobby предоставляет JSON API для обнаружения версий и использует `prerelease` (без дефиса) в URL.
...
| Тип | URL |
|-----|-----|
| Полная сборка | `https://cobylobbyht.store/launcher/patches/linux/x64/prerelease/0/22.pwr` |
| Diff 21→22 | `https://cobylobbyht.store/launcher/patches/linux/x64/prerelease/21/22.pwr` |
| Список версий | `https://cobylobbyht.store/launcher/patches/prerelease/versions?os_name=linux&arch=x64` |

> Обратите внимание, как `{branch}` разрешился в `prerelease` (а не `pre-release`) благодаря `branchMapping`.

### ShipOfYarn (json-index)

ShipOfYarn предоставляет единый API, возвращающий полный индекс файлов с прямыми URL для скачивания.
