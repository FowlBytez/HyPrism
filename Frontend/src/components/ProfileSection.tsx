...
export const ProfileSection: React.FC<ProfileSectionProps> = memo(({
  ...
  useEffect(() => {
    ipc.profile.get().then(p => {
      if (p.avatarPath) setLocalAvatar(p.avatarPath);
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    if (uuid) {
      ipc.profile.get().then(p => {
        setLocalAvatar(p.avatarPath ?? null);
      }).catch(() => {});
    }
  }, [uuid, refreshTrigger]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ipc.profile.get().then(p => {
        if (p.avatarPath && p.avatarPath !== localAvatar) {
          setLocalAvatar(p.avatarPath);
        }
      }).catch(() => {});
    }, 5000);
